...
function split_nn_ham(ham::QOperator)
	is_nn(Base.keys(ham)) || error("splt nn requires a nearest neighbour hamiltonian.")
	ham_even = typeof(ham)(space(ham))
	ham_odd = typeof(ham)(space(ham))
...
function propagator_2order(ham::QOperator, tf::Number, ti::Number)
	ks = keys(ham)
...
function propagator_2order(ham::QOperator, dt::Number)
	ks = keys(ham)
	if is_lower_than(ks, 2)
		if is_nn(ks)
		    ham_A, ham_B = split_nn_ham(ham)
...
function _propagator_impl(ham::QOperator, tf::Number, ti::Number, order::Int=1)
...
"""
	trotter_propagator(ham::QOperator, tspan::Tuple{<:Number, <:Number}; order::Int=2, dt::Number=0.01)
	algorithm reference: "Higher order decompositions of ordered operator exponentials"
"""
function trotter_propagator(ham::QOperator, tspan::Tuple{<:Number, <:Number}; order::Int=2, stepsize::Number=0.01)
...
"""
function infinite_trotter_propagator(ham::QOperator, tspan::Tuple{<:Number, <:Number}; period::Int=1, len::Int=length(ham), kwargs...)
	...
	nperiod = len ÷ period
	ham2 = QOperator([shift(item, (i-1) * period) for item in qterms(ham) if !(all(positions(item) .> period)) for i in 1:nperiod])
